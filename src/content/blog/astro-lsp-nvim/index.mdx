...
Any path with a lua file under `nvim/lsp/` is a 0.11 LSP configuration, and holds the solution to our woes. Reading the docs can suck it.

## The Solution
Neovim 0.11 provides a function pointer equivalent to `nvim-lspconfig`'s `on_new_config`: <Link href="https://neovim.io/doc/user/lsp.html#vim.lsp.ClientConfig">`before_init`</Link>. It's briefly mentioned in the official documentation but does't really exist anywhere outside of that.
...
return {
  ...
  before_init = function(_, config)
    if not config.init_options.typescript.tsdk then
      require("snacks").notifier.notify("Attaching astro")
